...
function unbucket(buckets) {
    ...
    return {positions, indices, edgeIndices};
}
...
function doCheckResult(buckets, mesh) {
    ...
    buckets.forEach(bucket => {
        ...
        for (let i = 0, len = indices.length; i < len; i += 3) {
            const key = positions[indices[i] * 3] + "_" + positions[indices[i] * 3 + 1] + "_" + positions[indices[i] * 3 + 2] + "/" +
        ...
        for (let i = 0, len = edgeIndices.length; i < len; i += 2) {
            const key = positions[edgeIndices[i] * 3] + "_" + positions[edgeIndices[i] * 3 + 1] + "_" + positions[edgeIndices[i] * 3 + 2] + "/" +
