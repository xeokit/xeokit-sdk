...
export class DTXTrianglesEdgesColorRenderer {
    ...
    constructor(scene) {
        ...
        this._appendVertexOutputs = (src, color, pickColor, gl_Position) => src.push(`vColor = vec4(vec3(${color}.rgb) * 0.5, float(${color}.a)) / 255.0;`);
        this._appendFragmentDefinitions = (src) => {
            src.push("in vec4 vColor;");
            src.push("out vec4 outColor;");
        };
        this._needvWorldPosition = false;
        this._appendFragmentOutputs = (src, vWorldPosition) => src.push("   outColor = vColor;");
