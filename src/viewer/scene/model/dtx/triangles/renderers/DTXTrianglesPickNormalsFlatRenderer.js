...
export class DTXTrianglesPickNormalsFlatRenderer {
    ...
    _bindProgram() {
        ...
        const scene = this._scene;
        if (scene.logarithmicDepthBufferEnabled) {
            const logDepthBufFC = 2.0 / (Math.log(scene.camera.project.far + 1.0) / Math.LN2);
            scene.canvas.gl.uniform1f(this._uLogDepthBufFC, logDepthBufFC);
        }
    }
    ...
    _buildVertexShader() {
        ...
        src.push("// DTXTrianglesPickNormalsFlatRenderer vertex shader");
    ...
    _buildFragmentShader() {
        ...
        const src = [];
        src.push('#version 300 es');
        src.push("// DTXTrianglesPickNormalsFlatRenderer fragment shader");
        src.push("#ifdef GL_FRAGMENT_PRECISION_HIGH");
        src.push("precision highp float;");
        src.push("precision highp int;");
        src.push("#else");
