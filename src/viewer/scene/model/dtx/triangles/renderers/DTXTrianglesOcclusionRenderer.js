...
export class DTXTrianglesOcclusionRenderer {
    ...
    drawLayer(frameCtx, dataTextureLayer, renderPass) {
        ...
        if (ENABLE_LOG_DEPTH_BUF && scene.logarithmicDepthBufferEnabled) {
            // TODO: Far from pick project objectInstanceMatrix?
    ...
    _buildVertexShader() {
        ...
        src.push("// DTXTrianglesOcclusionRenderer vertex shader");
    ...
    _buildFragmentShader() {
        ...
        const src = [];
        src.push('#version 300 es');
        src.push("// DTXTrianglesOcclusionRenderer fragment shader");
        src.push("#ifdef GL_FRAGMENT_PRECISION_HIGH");
        src.push("precision highp float;");
        src.push("precision highp int;");
        src.push("#else");
