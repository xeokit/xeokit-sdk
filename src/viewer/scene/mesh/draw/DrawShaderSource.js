    ...
    return {
        ...
        appendFragmentOutputs: (src, getGammaOutputExpression, gl_FragCoord) => {
            ...
            if (hasNonAmbientLighting) {
                ...
                if (specularMaterial) {
                    src.push("diffuseColor *= (1.0 - max(max(specular.r, specular.g), specular.b));"); // Energy conservation
                    src.push("float specularRoughness = clamp(1.0 - glossiness, 0.04, 1.0);");
                }

                if (metallicMaterial) {
                    src.push("float dielectricSpecular = 0.16 * specularF0 * specularF0;");
                    src.push("diffuseColor *= (1.0 - dielectricSpecular) * (1.0 - metallic);");
                ...
                if (phongMaterial || metallicMaterial || specularMaterial) {
                    ...
                    if (lightSetup.getReflection) {
                        ...
                        const spec = (phongMaterial
                                      ...
                                      : (function() {
                                          ...
                                          const specularBRDFContrib = `${BRDF_Specular_GGX_Environment}(viewNormal, viewEyeDir, specular, specularRoughness)`;
                    ...
                    lightSetup.directionalLights.forEach((light, i) => {
                        ...
                        const spec = (phongMaterial
                                      ...
                                      : `irradiance${i} * PI * ${BRDF_Specular_GGX}(lightDirection${i}, viewNormal, viewEyeDir, specular, specularRoughness)`);
                        src.push(`reflSpec += ${spec};`);
                    });
                }

                const ambient = phongMaterial && `${lightSetup.getAmbientColor()} * diffuseColor`;
                src.push("vec3 outgoingLight = emissiveColor + occlusion * (reflDiff * diffuseColor + reflSpec)" + (ambient ? (" + " + ambient) : "") + ";");
            } else {
