...
import {DepthShaderSource} from "./DepthShaderSource.js";
 ...
 * @private
 */
const DepthRenderer = function (hash, mesh) {
    this.id = ids.addItem({});
    this._hash = hash;
    this._scene = mesh.scene;
    this._useCount = 0;
    this._shaderSource = new DepthShaderSource(mesh);
    this._allocate(mesh);
};

const drawRenderers = {};

DepthRenderer.get = function (mesh) {
    ...
    if (!renderer) {
        renderer = new DepthRenderer(hash, mesh);
        if (renderer.errors) {
...
DepthRenderer.prototype.put = function () {
    if (--this._useCount === 0) {
...
DepthRenderer.prototype.webglContextRestored = function () {
    this._program = null;
};

DepthRenderer.prototype.drawMesh = function (frameCtx, mesh) {
    if (!this._program) {
        this._allocate(mesh);
    }
...
DepthRenderer.prototype._allocate = function (mesh) {
...
DepthRenderer.prototype._bindProgram = function (frameCtx) {
