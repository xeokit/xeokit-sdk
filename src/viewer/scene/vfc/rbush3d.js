    ...
    return (function () {
        ...
        1: [function (require, module, exports) {
            ...
            var enlargedVolume = function (a, b) {
                var minX = Math.min(a.minX, b.minX), minY = Math.min(a.minY, b.minY), minZ = Math.min(a.minZ, b.minZ),
            ...
            var intersectionVolume = function (a, b) {
                var minX = Math.max(a.minX, b.minX), minY = Math.max(a.minY, b.minY), minZ = Math.max(a.minZ, b.minZ),
                    maxX = Math.min(a.maxX, b.maxX), maxY = Math.min(a.maxY, b.maxY), maxZ = Math.min(a.maxZ, b.maxZ);
            ...
            var RBush3D = (function () {
                ...
                RBush3D.prototype.analyzeTriangles = function (node) {
                    ...
                    if (isLeaf(node)) {
                        for (var i = 0, len = node.children.length; i < len; i++) {
                            totalTriangles += node.children [i].numPrimitives;
                        }
                    } else {
                        for (var i = 0, len = node.children.length; i < len; i++) {
                ...
                RBush3D.prototype.groupNodesByNumTrianglesThreshold = function (numPrimitivesThreshold, node) {
                    if (node === undefined) {
                    ...
                    for (var i = 0, len = node.children.length; i < len; i++) {
                        ...
                        items.push({
                            item: child,
                            triangles: child.totalTriangles || child.numPrimitives,
                        });
                    }
                    ...
                    for (var i = 0, len = items.length; i < len; i++) {
                        ...
                        if ((retValItemTris + item.triangles) < numPrimitivesThreshold) {
                        ...
                        if (isLeafChild(node, item.item)) {
                            ...
                            if (tmp.length > 1) {
                                ...
                                for (var j = 0, len2 = tmp.length; j < len2; j++) {
                                    ...
                                    if (accum2 < numPrimitivesThreshold) {
                                        tmp2.push(tmp[j]);
                                    } else {
                                        if (accum2 == 0) {
                                            tmp2.push(tmp[j]);
                ...
                RBush3D.prototype.build = function (items, left, right, height) {
                    ...
                    node.leaf = false;
                    node.height = height;
                    var N3 = Math.ceil(N / M), N2 = N3 * Math.ceil(Math.pow(M, 2 / 3)),
